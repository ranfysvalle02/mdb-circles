...
`;
    ...
    // Start polling for webrtc posts
    startWebRTCPostPolling(container);
}

// Polling mechanism for webrtc posts
let webrtcPostPollingInterval = null;

function startWebRTCPostPolling(container) {
    // Clear existing interval if any
    if (webrtcPostPollingInterval) {
        clearInterval(webrtcPostPollingInterval);
    }
    
    // Poll every 5 seconds for webrtc posts
    webrtcPostPollingInterval = setInterval(() => {
        updateWebRTCPosts(container);
    }, 5000);
...
async function updateWebRTCPosts(container) {
