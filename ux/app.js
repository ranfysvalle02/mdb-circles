...
class ActivityPoller {
    ...
    async poll() {
        ...
        try {
            let endpoint = '/users/me/activity-status';
            if (this.lastCheckedTimestamp) {
                endpoint += `?since=${encodeURIComponent(this.lastCheckedTimestamp)}`;
            }

            const data = await apiFetch(endpoint);

            // 1. Update timestamp for the next poll
            this.lastCheckedTimestamp = data.new_server_timestamp;

            // 2. Update FAB badge with total counts from the response
            this.updateFab(data.new_invites_count, data.new_notifications_count);

            // 3. Handle new comment activity, which is the 'delta'
            if (data.new_comment_activity && data.new_comment_activity.length > 0) {
                this.handleNewCommentActivity(data.new_comment_activity);
            }

        } catch (error) {
            console.error("Activity poll failed:", error);
            // Don't stop polling on a single network failure.
            // If it's a 401, apiFetch will handle logout and the poller will stop.
        }
    }
    ...
    handleNewCommentActivity(activity) {
        ...
        activity.forEach(item => {
            ...
            // Only update the UI if the post is currently visible to the user
            if (postWrapper) {
                ...
                // Update the comment count badge on the post
        ...
        // Show a single, consolidated toast notification for all new comment activity
...
function renderNotificationsList(notifications) {
    ...
    container.innerHTML = notifications.map(n => {
        let contentHtml = '';
        let notificationLink = '#'; // Default link
        ...
        switch(n.type) {
            ...
            case 'new_comment':
                ...
                // We can make the notification clickable to open the comments modal
                if (n.content.post_id) {
                    // This is a bit of a trick to make the whole item clickable later
        ...
        // Make the entire card a link if a link is available
        if (notificationLink !== '#') {
...
async function handleRoute() {
  ...
  if (state.currentUser) {
    ...
    activityPoller.stop(); // Ensure poller is stopped if user logs out
  }
...
`;
 ...
 in <a href="#/circle/${post.circle_id}"
  ...
  ${displayCircleName}
 </a> &bull; ${new Date(post.created_at).toLocaleString()}
 </small>
 </div>
 </div>
...
async function handlePollVote(postId, optionIndex) {
  try {
    ...
    if (postWrapper) {
      ...
      if (footer) {
        const timePart = footer.innerHTML.includes('&bull;') ?
          footer.innerHTML.split('&bull;')[1] :
          '';
        footer.innerHTML = `Total votes: ${totalVotes} ${timePart ? `&bull;${timePart}` : ''}`;
      }
    }
  } catch (e) {
