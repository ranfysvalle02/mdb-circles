...
// NEW: State for chat feature during post creation
chat: {
 is_enabled: false,
 participant_ids: []
}
...
if (!circleActivity.has(event.circle_id)) {
 ...
 if (circle) circleActivity.set(event.circle_id, {
 name: circle.name,
 actors: new Set()
 });
}
...
// NEW: Conditionally add the chat button
...
// This function now finalizes the post after all data (including chat) is ready
...
try {
 ...
 case 'wishlist': {
  ...
  payload.text = title;
  payload.wishlist_data = urls.map(url => ({ url, title: new URL(url).hostname }));
  break;
 }
 case 'playlist': {
 ...
 case 'poll': {
  ...
  payload.poll_data = { question, options: options.map(text => ({ text })) };
 ...
 case 'image': {
  ...
  payload.image_data = { ...state.postCreation.imageData, caption };
  } else {
  payload.image_data = { url: imageUrl, caption };
  }
  break;
 }
 case 'spotify_playlist': {
...
// This function decides whether to post directly or open the chat selector
...
// Store whether chat is enabled in the state
state.postCreation.chat.is_enabled = creator.querySelector('#enableChatSwitch').checked;

if (state.postCreation.chat.is_enabled) {
// If chat is on, just open the modal to select members and stop.
 // The final posting action will be triggered from that modal's button.
 openChatParticipantSelector(circle_id);
} else {
// If chat is off, clear participants and proceed directly to posting.
...
async function openChatParticipantSelector(circleId) {
    ...
    try {
        ...
        // CORRECTED: Changed state.currentUser.id to state.currentUser._id
        // This now correctly filters out the person making the post.
        ...
        if (members.length === 0) {
            ...
            document.querySelector('[data-action="confirm-chat-participants"]').disabled = true;
            return;
        }
        document.querySelector('[data-action="confirm-chat-participants"]').disabled = false;
        container.innerHTML = members.map(member => `
<div class="form-check">
 <input class="form-check-input" type="checkbox" value="${member.user_id}" id="chat-member-${member.user_id}" checked>
...
// MODIFIED: apiFetch already shows the detailed error. We just log and reset the view.
...
async function handleOpenChatModal(postId) {
 ...
 // Attach postId to the form for the submit handler
...
// Helper to render the entire list of chat messages
...
// Helper to append a single new message
function appendChatMessage(message, container) {
  // If the placeholder is there, remove it first
...
// Helper to generate HTML for a single message bubble
...
'#notificationsModal',
'#helpModal',
'#chatParticipantSelectorModal', // NEW
'#chatModal' // NEW
].forEach(id => {
...
// NEW: Add submit listener for the chat form
...
switch (action) {
  // NEW: Handle opening the chat modal
  case 'open-chat':
...
case 'accept-invite':
 ...
 // MODIFIED: The apiFetch function automatically shows a status message for server errors
 // like 409 (Conflict). We can log the error for debugging but don't need to call showStatus() again.
