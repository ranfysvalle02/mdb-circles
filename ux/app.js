            ...
            return `
                <div class="list-group-item d-flex justify-content-between align-items-center">
                    ...
                    <div class="btn-group btn-group-sm">
                        <button class="btn btn-success" data-action="accept-friend" data-friend-id="${request.friend_id}">
                            <i class="bi bi-check"></i>
                        </button>
                        <button class="btn btn-danger" data-action="reject-friend" data-friend-id="${request.friend_id}">
                            <i class="bi bi-x"></i>
                        </button>
                    </div>
                </div>
...
async function acceptFriendRequest(friendId) {
    ...
    const response = await apiFetch(`/friends/${friendId}/accept`, {
        method: 'POST'
    });
    showStatus('Friend request accepted!', 'success');
    await loadFriends();
    // Refresh activity center to update notifications
    if (activityPoller) {
        activityPoller.poll();
    }
    return response;
...
async function rejectFriendRequest(friendId) {
    ...
    const response = await apiFetch(`/friends/${friendId}/reject`, {
        method: 'POST'
    });
           ...
           <div class="d-flex align-items-center gap-2 flex-wrap justify-content-end">
            ...
            if (gameWidgetsHtml) {
                ...
                if (!widgetsContainer) {
                    ...
                    widgetsContainer.className = 'mb-3 d-flex flex-wrap gap-2';
                    // Insert after circle header
                    const headerElement = dom.circleHeader;
                    if (headerElement && headerElement.parentNode) {
                        headerElement.parentNode.insertBefore(widgetsContainer, headerElement.nextSibling);
                    } else {
...
class CircleGameIntegration {
    ...
    // Uses /lobby/{circle_id}/{game_type} POST endpoint which automatically gets or creates
    async joinGame(gameType, gameMode = null, aiCount = null) {
        const requestBody = {
            player_id: this.userId
        };
        
        // Only include game_mode and ai_count if they are provided (not null)
        if (gameMode !== null && gameMode !== undefined) {
            requestBody.game_mode = gameMode;
        }
        ...
        try {
            // Try /lobby/{circle_id}/{game_type} POST
            ...
            if (!response.ok) {
                // Try /api/lobby/ if first attempt fails
                ...
                if (!altResponse.ok) {
                    // Both failed, try creating game directly
                    console.warn('Lobby endpoints failed, creating game directly...');
            ...
            if (result.error) {
                ...
                if (result.error.toLowerCase().includes('not found') || 
                    ...
                    console.log('Game not found in response, creating new game...');
                    return await this.createGameDirectly(gameType, gameMode, aiCount);
                }
                throw new Error(result.error);
            }
            
            // Validate that we got a game_id - CRITICAL!
            if (!result.game_id) {
                console.error('No game_id in response:', result);
                // Try to create game directly as fallback
                return await this.createGameDirectly(gameType, gameMode, aiCount);
            }
            ...
            console.warn('Error joining lobby, attempting to create new game:', error);
            try {
                return await this.createGameDirectly(gameType, gameMode, aiCount);
            } catch (createError) {
    ...
    // Format lobby result to match expected response format
    formatLobbyResult(result, gameType) {
        // CRITICAL: Ensure game_id exists
        if (!result || !result.game_id) {
            console.error('formatLobbyResult: Missing game_id in result:', result);
            throw new Error('Invalid response: game_id is required');
        }
    ...
    async createGameDirectly(gameType, gameMode = null, aiCount = null) {
        ...
        if (!createResponse.ok) {
            ...
            throw new Error(`Failed to create game: ${createResponse.status} ${createResponse.statusText}`);
        }
        ...
        if (!gameData || !gameData.game_id) {
            console.error('createGameDirectly: No game_id in response:', gameData);
            throw new Error('Failed to create game: No game_id returned');
        }
    ...
    async getLobbyStatus(gameType) {
        // Use /lobby/{circle_id}/{game_type} GET endpoint
        let response;
        try {
            response = await fetch(
                `${this.baseUrl}/lobby/${this.circleId}/${gameType}`
    ...
    async updateSettings(gameType, gameMode = null, aiCount = null) {
        ...
        // Use /lobby/{circle_id}/{game_type}/settings POST endpoint
        let response;
        let lastError;
        
        try {
            response = await fetch(
                `${this.baseUrl}/lobby/${this.circleId}/${gameType}/settings`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(requestBody)
            ...
            if (response.status === 404) {
                // Try /api/lobby/ if first attempt fails
                response = await fetch(
                    `${this.baseUrl}/api/lobby/${this.circleId}/${gameType}/settings`,
                    {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(requestBody)
...
async function handleStartGameLobby(circleId, gameType, gameMode = null, aiCount = null) {
    ...
    const button = document.querySelector(`[data-action="create-game-lobby"][data-circle-id="${circleId}"][data-game-type="${gameType}"]`) ||
                   document.querySelector(`[data-action="start-game-lobby"][data-circle-id="${circleId}"][data-game-type="${gameType}"]`);
    if (button) {
...
class GamePortalWidget {
    ...
    async updateUI() {
        ...
        try {
            ...
            const endpoint = `https://apps.oblivio-company.com/experiments/game_portal/backend/api/game/${this.gameId}/poll`;
    ...
    render(gameData) {
        if (!this.container) {
            console.warn('GamePortalWidget: Container not found for gameId:', this.gameId);
            return;
        }
        
        const { status, players, player_count, max_players, game_state_summary } = gameData;
        
        console.log('GamePortalWidget: Rendering game data:', {
            gameId: this.gameId,
            status,
            player_count,
            max_players,
        ...
        // Update player list - ensure we have valid values
        const validPlayerCount = player_count || 0;
        const validMaxPlayers = max_players || 4;
        this.updatePlayerList(players || [], validPlayerCount, validMaxPlayers);
    ...
    detectChanges(oldState, newState) {
        ...
        // Hand finished
        if (oldState.status === 'in_progress' && newState.status === 'hand_finished') {
    ...
    updatePlayerList(players, currentCount, maxCount) {
        ...
        if (playerListEl) {
            // Handle empty lobby - show empty state (magical!)
            if (validPlayers.length === 0) {
                ...
                playerListEl.innerHTML = validPlayers.slice(0, 4).map(p => {
                    ...
                    const badge = isAI ? '🤖' : isSpectator ? '👀' : '👤';
                    const displayName = isAI ? 'AutoBot' : this.formatPlayerId(pid);
                    return `<div class="player-item ${isAI ? 'ai-player' : ''}" title="${displayName}">
        ...
        // Update compact player count - magical: shows 0/4 when empty!
        ...
        if (regularCountEl) {
            if (currentCount === 0) {
                regularCountEl.classList.add('empty-lobby-count');
                regularCountEl.textContent = `✨ ${currentCount}/${maxCount} players - Join to become host!`;
            } else {
                regularCountEl.classList.remove('empty-lobby-count');
    ...
    formatPlayerId(playerId) {
        // Format player ID for display (per guide - filter out placeholder and temporary IDs)
    ...
    document.body.addEventListener('click', async e => {
        ...
        switch (action) {
            ...
            case 'accept-friend':
                {
                    ...
                    try {
                        ...
                        showStatus(error.message || 'Failed to accept friend request', 'danger');
                    } finally {
                        setButtonLoading(target, false);
                    }
                    break;
            ...
            case 'reject-friend':
                {
                    ...
                    try {
                        ...
                        showStatus(error.message || 'Failed to reject friend request', 'danger');
                    } finally {
                        setButtonLoading(target, false);
                    }
                    break;
            ...
            case 'join-game-lobby':
                {
                    ...
                    try {
                        ...
                        // Use CircleGameIntegration to join - Game Platform handles game settings
                        const gameIntegration = new CircleGameIntegration(circleId, playerId);
                        const joinResult = await gameIntegration.joinGame(gameType, null, null);
                        ...
                        // Open Game Portal - Game Platform handles game settings UI
                        const gameUrl = redirectUrl || 
            ...
            case 'update-lobby-settings':
                {
                    // Settings are handled in Game Portal - just redirect there
                    const circleId = data.circleId;
                    const gameType = data.gameType;
                    ...
                    try {
                        ...
                        // Join lobby to get game_id, then redirect to Game Portal
                        const gameIntegration = new CircleGameIntegration(circleId, playerId);
                        const joinResult = await gameIntegration.joinGame(gameType, null, null);
                        ...
                        // Redirect to Game Portal - settings are handled there
                        const gameUrl = redirectUrl || 
                                       `https://apps.oblivio-company.com/experiments/game_portal?game=${gameId}&replace_placeholder=true`;
                        window.open(gameUrl, '_blank');
                        showStatus('Opening Game Portal...', 'info');
                    } catch (error) {
                        console.error('Failed to join lobby:', error);
                        showStatus(error.message || 'Failed to join lobby', 'danger');
                    } finally {
                        setButtonLoading(target, false);
                    }
                    break;
            ...
            case 'open-yt-search':
                ...
                break;
            case 'open-start-game-modal':
                {
                    // No modal - Game Platform handles everything
                    // This handler is kept for backwards compatibility but does nothing
                    // Users should use create-game-lobby or join-game-lobby instead
                    break;
                }
            case 'create-game-lobby':
                {
                    const circleId = data.circleId;
                    const gameType = data.gameType;
                    ...
                    setButtonLoading(target, true);
                    try {
                        // Create/join game lobby - Game Platform handles game settings
                        await handleStartGameLobby(circleId, gameType, null, null);
